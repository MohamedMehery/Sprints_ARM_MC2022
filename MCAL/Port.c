...
void Port_Init(const Port_configType* ConfigPtr)
{
	...
	for (pin = 0; pin < TOTAL_PORT_NUMBER_OF_PINS; pin++)
	{
		...
		)
		{
			...
			*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOLOCK_OFFSET) = 0x4C4F434B;
			
			/* Configure GPIOCR bits to allow changes on the dedicated pin */
			SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOCR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
		}
		...
		switch (ConfigPtr->pinConfigType[pin].pinModeType)
		{
			case DIO_MODE:
				...
				CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOAMSEL_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
				
				/* Clear the corresponding bit to the dedicated pin in GPIOAFSEL */
				CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOAFSEL_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
				
				/* Clear the PMCx bits for the dedicated pin */
				*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPCTL_OFFSET) &= ~(0x0000000F << (ConfigPtr->pinConfigType[pin].pinType * 4));
				
				/* Set the corresponding bit of the dedicated pin in GPIODEN register to enable the digital functionality to this pin */
				SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODEN_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
				break;
			case ICU_MODE:
		...
		if (ConfigPtr->pinConfigType[pin].pinDirectionType == OUTPUT)
		{
			...
			SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODIR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
			
			switch (ConfigPtr->pinConfigType[pin].pinOutputCurrentType)
			{
				case DRIVE_2mA:
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR2R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR4R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR8R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				case DRIVE_4mA:
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR2R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR4R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR8R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				case DRIVE_8mA:
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR2R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR4R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR8R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				default:
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR2R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODR4R_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
			...
			if (ConfigPtr->pinConfigType[pin].pinLevelType == PIN_LEVEL_HIGH)
			{
				...
				SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODATA_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
			}
			else
			{
				/* Clear the corresponding bit of the dedicated pin in GPIODATA */
				CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODATA_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
			}
		}
		else if (ConfigPtr->pinConfigType[pin].pinDirectionType == INPUT)
		{
			/* Set the corresponding bit of the dedicated pin in GPIODIR */
			CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIODIR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
		
			switch (ConfigPtr->pinConfigType[pin].pinInternalAttachType)
			{
				case PULL_UP:
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPUR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPDR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOODR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOSLR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				case PULL_DOWN:
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPUR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPDR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOODR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOSLR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				case OPEN_DRAIN:
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPUR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPDR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOODR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOSLR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
				case SLEW_RATE:
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPUR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOPDR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					CLEAR_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOODR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					SET_BIT(*(volatile uint32*)((volatile uint8_t*)Portptr + GPIO_GPIOSLR_OFFSET), ConfigPtr->pinConfigType[pin].pinType);
					break;
