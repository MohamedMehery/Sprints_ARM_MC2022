...
void IntCrtl_Init(void)
{
	...
	for (irq = 0; irq < MICROCONTROLLER_INTERRUPTS_NUM; irq++)
	{
		...
		else if (ConfigPtr->interrupts[irq].mode == INT_ENABLED)
		{
			...
			irq_id = ConfigPtr->interrupts[irq].interruptId;
			uint8_t PrixOffset = irq_id / 4;		/* integer value */
			uint8_t Intx = irq_id % 4;	/* onteger value from 0 to 3 corresponding to INTA, INTB, INTC, and INTD */
			volatile NVIC_PRIn_BF* PRIx = (volatile NVIC_PRIn_BF*)((volatile uint8_t*)NVIC_PRIx_BASE + (PrixOffset*4));
...
{
	/* Assign the priority to every interrupt */
	uint8_t irq_id = InterruptId;
	uint8_t PrixOffset = irq_id / 4;		/* integer value */
	uint8_t Intx = irq_id % 4;	/* onteger value from 0 to 3 corresponding to INTA, INTB, INTC, and INTD */
	volatile NVIC_PRIn_BF* PRIx = (volatile NVIC_PRIn_BF*)(CORTEXM4_PERI_BASE_ADDRESS + (uint8_t*)NVIC_PRIx_BASE + (PrixOffset*4));
